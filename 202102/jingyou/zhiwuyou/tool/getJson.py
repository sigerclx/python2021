...
def produce3(s,danfangName,searchheader=searchheader):
    ...
    data =data['data']
    f1.write('\n功效:')
    f1.write('\n可用于香薰:' + str(data['aromatically']))
    f1.write('\n可用于涂抹:' + str(data['topically']))
    f1.write('\n可用于口服:' + str(data['internally']))

    healingAttributePhysiologyList = data['healingAttributePhysiologyList']
    f1.write("\n疗愈属性：\n")
    f1.write("\n生理方面：\n")
    ...
    try:
        f1.write("\n心理方面：\n")
        healingAttributeMentalityList = data['healingAttributeMentalityList']
    ...
    f1.write("\n用途：\n")
    healthConditionList = data['healthConditionList']
    for i in range(len(healthConditionList)):
        f1.write("\n"+healthConditionList[i]['typeBaseKindCnName']+":\n")
        healthConditionList1 = data['healthConditionList'][i]['healthConditionList']
