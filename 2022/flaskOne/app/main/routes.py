...
def index():
    ...
    first_url = url_for('main.index', baseDict=baseDict, page=1)
    next_url = url_for('main.index', baseDict=baseDict, page=reimbursements.next_num) \
        if reimbursements.has_next else None
    prev_url = url_for('main.index', baseDict=baseDict, page=reimbursements.prev_num) \
        if reimbursements.has_prev else None
    # baseDict=baseDict 是否可以不在这传递？
    last_url = url_for('main.index', page=Allpages)
