import os,datetime
import backup5.tools.stringfunc
...
def likedate(folder):
    ...
    if backup5.tools.stringfunc.is_valid_date(foldername):
        return 1
    else:
        return 0

def protectdays(folder,backupinfo):
# 如果是保护目录，则返回.判别原则是目录长路径里是否含有日期字符串
...
def compressdays(folder,backupinfo):
# 如果是保护目录，则返回.判别原则是目录长路径里是否含有日期字符串
    compressdays = __get_copy_dates(backupinfo.protectDays)
    foldershortName = folder[folder.rfind("\\") + 1:]
    for compressday in compressdays:
        #print(foldershortName,compressday)
        if compressday == foldershortName:
            print('\n----- 要压缩的目标目录：', folder, ' ------')
            return 1
    return 0
